...
public enum ExtendedDNSError : ushort
{
    ...
    /// Implementations SHOULD include an EXTRA-TEXT value to augment this error code with additional information.
    /// </summary>
    Other = 0x0,
    ...
    /// This is typically caused by problems communicating with an authoritative server, possibly as result of a denial of service (DoS) attack against another network.
    /// </summary>
    StaleAnswer = 0x3,
    ...
    /// <summary>
    /// The server is unable to respond to the request because the domain is on a blocklist due to an internal security policy imposed by
    /// the operator of the server resolving or forwarding the query.
    /// </summary>
    Blocked = 0xF,

    /// <summary>
    /// The server is unable to respond to the request because the domain is on a blocklist due to an external requirement imposed
    /// by an entity other than the operator of the server resolving or forwarding the query. 
    /// </summary>
    Censored = 0x10,
    ...
    /// <summary>
    /// The resolver was unable to resolve an answer within its configured time limits and decided to answer with a previously
    ...
    /// for a domain for which it is not authoritative, SHOULD include this EDE code in the REFUSED response.
    /// </summary>
